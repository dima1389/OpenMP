/*
 * File:        omp_schedule_profile_used_threads.c
 *
 * Purpose:
 *   Refines omp_schedule_profile.c by printing statistics only for the *actually used*
 *   number of threads in the parallel region (omp_get_num_threads()) rather than the
 *   maximum possible threads (omp_get_max_threads()).
 *
 *   The program profiles how OpenMP scheduling distributes loop work across threads:
 *     - schedule(static,  chunk)
 *     - schedule(dynamic, chunk)
 *     - schedule(guided,  chunk)
 *     - schedule(runtime) (controlled via OMP_SCHEDULE)
 *
 *   It collects per-thread statistics:
 *     - number of iterations executed
 *     - total work units (a deterministic proxy for relative effort)
 *
 * Key concepts:
 *   - Actual team size vs maximum available threads
 *   - Scheduling policy and chunk size effects on load balance
 *   - Per-thread accounting with deterministic, low-overhead measurement
 *
 ...
 *       omp_schedule_profile_used_threads.c -o omp_schedule_profile_used_threads
 *
 * Execution:
 *   ./omp_schedule_profile_used_threads [N] [pattern] [chunk]
 *
 *   Arguments:
 ...
 *
 * Examples:
 *   ./omp_schedule_profile_used_threads 20000000 1 1
 *   ./omp_schedule_profile_used_threads 20000000 1 1024
 *
 *   export OMP_SCHEDULE="dynamic,4096"
 *   ./omp_schedule_profile_used_threads 20000000 3 64
 *
 * Notes:
 *   - Work units are synthetic and deterministic; they approximate iteration cost.
 *   - This program avoids heavy per-iteration computation to minimize profiling noise.
 *
 * References:
...
/*
 * Runs one profiling pass with a chosen schedule, using exactly the current
 * team size (omp_get_num_threads()) and printing only those threads.
 */
static double run_profiled_pass(omp_sched_t kind,
                                long long n, int pattern, int chunk,
                                const char *label)
{
    double elapsed = 0.0;

    #pragma omp parallel default(none) shared(n, pattern, chunk, kind, label, elapsed)
    {
        static thread_stats_t *stats = NULL;
        static int used_threads = 0;
        ...
        #pragma omp single
        {
            ...
            stats = (thread_stats_t *)calloc((size_t)used_threads, sizeof(thread_stats_t));
            if (stats == NULL) {
                fprintf(stderr, "Allocation failure for per-thread stats.\n");
                exit(1);
            }
            stats_zero(stats, used_threads);
        ...
        #pragma omp single
        {
            ...
            stats_print(label, stats, used_threads);
...
int main(int argc, char *argv[])
{
    ...
    printf("OpenMP scheduling profiler (prints only used threads)\n");
    ...
    double t_static = run_profiled_pass(omp_sched_static, n, pattern, chunk, "Schedule: static");
    printf("Elapsed time (static):  %.6f s\n\n", t_static);

    double t_dynamic = run_profiled_pass(omp_sched_dynamic, n, pattern, chunk, "Schedule: dynamic");
    printf("Elapsed time (dynamic): %.6f s\n\n", t_dynamic);

    double t_guided = run_profiled_pass(omp_sched_guided, n, pattern, chunk, "Schedule: guided");
    printf("Elapsed time (guided):  %.6f s\n\n", t_guided);

    double t_runtime = run_profiled_pass(omp_sched_auto, n, pattern, chunk,
                                         "Schedule: runtime (set via OMP_SCHEDULE)");
